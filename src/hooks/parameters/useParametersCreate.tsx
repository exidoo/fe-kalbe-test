'use client';

// import useMutation dari '@tanstack/react-query'
import { useMutation, useQueryClient } from '@tanstack/react-query';

// import service API
import Api from '@/services/api';

// import js-cookie
import Cookies from 'js-cookie';

// import toast dari 'react-hot-toast'
import toast from 'react-hot-toast';

// interface untuk request data
// Menghilangkan field yang di-generate otomatis oleh server (id, created_on, dll)
export interface AddParameterRequest {
  code?: string;
  description?: string;
  created_by_id?: number;
  is_active?: boolean;
}

// useParametersCreate Hook
export const useParametersCreate = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (data: AddParameterRequest) => {
      const token = Cookies.get('token');

      const response = await Api.post('/parameters', data, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      return response.data;
    },
    onSuccess: () => {
      // Invalidate cache untuk query 'parameters' agar daftar parameter otomatis terupdate
      queryClient.invalidateQueries({ queryKey: ['parameters'] });
      // Tampilkan notifikasi sukses menggunakan react-hot-toast
      toast.success('Parameter berhasil ditambahkan!');
    },
    onError: (error) => {
      // Tampilkan notifikasi error menggunakan react-hot-toast
      toast.error(`Gagal menambahkan parameter.`);
      console.error(error); // Log error untuk debugging
    },
  });
};
